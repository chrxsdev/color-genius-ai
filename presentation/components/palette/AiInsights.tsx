...
export const AiInsights = ({ rationale, tags, containerClassName = '' }: AiInsightsProps) => {
  ...
  return (
    <div className={`mt-2 p-3 sm:p-6 rounded-xl bg-neutral-variant/20 border-2 border-neutral-variant/50 space-y-4 ${containerClassName}`}>
      {rationale && (
        <div>
      ...
      {tags.length > 0 && (
        <div>
          ...
          <div className='flex flex-wrap gap-1.5 sm:gap-2'>
            {tags.map((tag) => (
              <span key={tag} className='px-2 sm:px-3 py-1 rounded-full bg-primary/20 text-primary text-[10px] sm:text-xs font-medium'>
                {tag}
              </span>
            ))}
          </div>
