...
An AI-first app that generates usable, web-ready color palettes from a user’s text natural language. The AI returns a complete creative package (palette HEX codes, names, rationale, tags, gradient, and font suggestions). The client verifies and gently corrects the AI output using color-science rules, then applies user-controlled sliders for brightness, saturation, and warmth. Users visualize colors on a wheel, preview a gradient or UI element, and copy Tailwind/CSS snippets.
...
- Future-ready
  - Tags enable browsing and recommendations later.
  - Font suggestions and gradient provide immediate practical usage.
  - Deterministic options (seed) and shareable saved palettes via Supabase.